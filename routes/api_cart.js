...
router.post("/cart", async function (req, res, next) {
    let listC = await cartController.getListCart(req.decode.email);
...
router.post("/create-order", auth.checkMobileAuthentication, async function (req, res, next) {
    ...
    const { products, note, total, value, superTotal } = req.body;
    let list = [];
    for (const e of products) {
        list.push({ name: e.name, quantity: e.quantity });
    }
    let newOrder = {
        user_id: user_id,
        status: "Dangxuli",
